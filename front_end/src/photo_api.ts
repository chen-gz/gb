...
// }
// export async function  addPhoto2(jpeg_file?: File, ori_file?: File) {
//     // first get the photo from the server.
//     // if the photo is not exist, then upload the photo to the server
//     // if the photo exist, test the photo link valid of not.
//     // if the photo original is false, but the original file is exist, then upload the original file to the server and
//     // update the photo original to true
//
//     let ori_hash, jpeg_hash;
//     if (ori_file != null) {
//         // calculate hash of orifile
//         ori_hash = ori_file.arrayBuffer().then((buffer) => {
//             const uint8array = new Uint8Array(buffer);
//             return calculateSHA1(uint8array);
//         })
//     }
//     // calculate hash of jpegfile
//     if (jpeg_file != null) {
//         jpeg_hash = jpeg_file.arrayBuffer().then((buffer) => {
//             const uint8array = new Uint8Array(buffer);
//             return calculateSHA1(uint8array);
//         })
//     }
//     // await for the hash calculation
//     await Promise.all([ori_hash, jpeg_hash])
